...
public final class ActivityNavigator {
	...
	public static final NavSettings DF_SETTINGS = new NaveSettingsBuilder().isAnimated(true).build();
	public static final NavSettings B_REF_SETTINGS = new NaveSettingsBuilder().clearStack(true).isReference(true)
																			  .build();
	...
	public void navigate(Fragment fragment, NavSettings settings) {
		...
		transaction.replace(container, fragment, String.valueOf(fCount)).commit();
	}
	...
	String saveInstanceState() {
		...
		stringBuilder.append(fCount).append('&');
	...
	public static class NavSettings {
		...
		public NaveSettingsBuilder toBuild() {
			return new NaveSettingsBuilder().clearStack(clearStack).isReference(isReference).isAnimated(isAnimated);
		}
